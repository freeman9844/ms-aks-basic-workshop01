...
Managed Grafana/Prometheus와 Container Insights로 클러스터를 **메트릭(시계열)** 과 **로그(이벤트)** 양쪽에서 관측합니다. 앞 모듈(07~08)에서 만든 오토스케일링 동작을 실제 그래프와 로그로 확인하는 것이 이 모듈의 목표입니다.
...
- **Namespace (Workloads)** 대시보드에서 namespace를 `pets`로 필터링하면, 모듈 07에서 `orders` 큐가 쌓이며 `makeline-service`의 **Pod 수가 늘어났다가(KEDA 스케일아웃)**, 큐가 비면 다시 **0~1개로 축소**되는 모습을 볼 수 있습니다.
...
아래는 Explore에서 `count(kube_pod_info{namespace="pets"})` 쿼리로 `pets` Pod 개수 변화를 본 예시입니다. 모듈 07~08의 스케일아웃/스케일인이 Pod 수 그래프로 그대로 나타납니다.
...
Kubernetes 이벤트(스케줄링/이미지 풀/오토스케일 등 — 모듈 07~08 추적에 유용):
```kql
KubeEvents
...
> **메트릭 vs 로그 비용 감각:** Container Insights 로그는 수집량(GB)에 비례해 과금됩니다. 운영에서는 `ContainerLogV2`의 네임스페이스/스트림 필터링과 LAW **데이터 보존 기간** 조정으로 비용을 통제합니다. 실습 후에는 모듈 10에서 모든 백엔드가 함께 삭제됩니다.

---

## 검증 및 완료 체크리스트

다음 항목이 모두 충족되면 [10. 정리](10-cleanup.md)로 진행하세요.
...
- [ ] 모듈 07~08의 스케일링 이벤트가 그래프(또는 `KubeEvents`)에 반영됨
...
메트릭·로그에 더해 **분산 추적(트레이스)** 까지 코드 수정 없이 체험하고 싶다면 아래 **옵션** 모듈로 진행할 수 있습니다.

> 🧩 **(옵션) 시간 여유가 있을 때** → **[09.1 (옵션) 애플리케이션 자동 계측(Auto-instrumentation)](09.1-autoinstrument-option.md)**
> 코드 수정 없이 OpenTelemetry 에이전트를 주입해 `order-service`(Node.js)의 **분산 추적**을 Application Insights에서 확인합니다. 필수는 아니며, 빡빡하면 건너뛰어도 됩니다.
