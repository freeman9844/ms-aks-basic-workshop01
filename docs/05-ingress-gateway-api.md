...
istiod-asm-xxxxx-7d9f8c6b5-fghij              1/1     Running   0          2m

$ kubectl get gatewayclass approuting-istio
NAME               CONTROLLER                               ACCEPTED   AGE
approuting-istio   istio.aks.azure.com/gateway-controller   True       2m
```
> `istiod`는 가용성을 위해 **기본 2 레플리카**(HPA min 2~max 5, PDB min 1)로 뜹니다. 컨트롤 플레인은 GA 기준 AKS 버전에 맞춰 **in-place**로 업그레이드됩니다.
...
>
> Gateway 하나당 AKS가 **Envoy 프록시 Deployment + Service + HPA + PDB**(모두 `store-gateway-approuting-istio` 이름)를 자동 생성·관리합니다(`kubectl get deploy,svc,hpa,pdb -n pets | grep approuting-istio`). 기본 HPA는 2~5 레플리카, PDB는 min 1이라 업그레이드/통합 중에도 1개는 유지됩니다 — 이 PDB는 [08. NAP](08-autoscaling-nap.md) 노드 통합 시 빈-노드 회수만 허용되는 이유와도 연결됩니다.
