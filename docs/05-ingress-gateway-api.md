...
> - `--enable-gateway-api` (Gateway API CRD 설치): 약 2–4분
> - `--enable-app-routing-istio` (Istio 컨트롤 플레인 구성 + `istiod` 기동): 약 4–8분
>
> 두 명령 모두 완료될 때까지 터미널이 블로킹됩니다. 기다리는 동안 아래 3) Gateway/HTTPRoute 개념 설명을 미리 읽어 두면 좋습니다.
>
> ⚠️ **두 `az aks update`는 반드시 순서대로, 앞 작업이 끝난 뒤 실행하세요.** 앞 작업이 진행 중일 때 다음 명령을 실행하면 `(EtagMismatch) Another operation is in progress` 오류가 납니다. (NAP/오토업그레이드 등 백그라운드 작업과 겹쳐도 동일) 각 명령 사이에 아래 대기 명령으로 `Succeeded` 상태를 확인하면 안전합니다.
...
```
> 예상: 클러스터 업데이트가 진행되며 약 2–4분 걸립니다. 완료되면 갱신된 클러스터 JSON이 반환됩니다.
...
이제 Istio 기반 app routing 애드온을 활성화합니다. **그 전에 앞 작업이 완료(`Succeeded`)됐는지 확인**합니다.
```bash
# 앞의 update가 끝날 때까지 대기 (다른 작업이 진행 중이면 완료까지 블로킹)
az aks wait -g "$RG" -n "$AKS" --updated --interval 15 --timeout 1200

# 또는 현재 상태만 즉시 확인 (Succeeded 여야 다음 명령 실행 가능)
az aks show -g "$RG" -n "$AKS" --query provisioningState -o tsv

az aks update -g "$RG" -n "$AKS" --enable-app-routing-istio
```
> 예상: Istio 컨트롤 플레인 구성으로 약 4–8분 소요됩니다. 완료되면 클러스터 JSON이 반환되고, `aks-istio-system` 네임스페이스에 `istiod` 컨트롤 플레인이 기동됩니다.
...
| 증상 | 원인 | 진단 | 조치 |
|---|---|---|---|
| `(EtagMismatch) Another operation is in progress` | 앞 `az aks update`(또는 NAP/오토업그레이드 등)가 아직 진행 중인데 다음 명령을 실행 | `az aks show -g $RG -n $AKS --query provisioningState -o tsv` (≠ `Succeeded`이면 진행 중) | `az aks wait -g $RG -n $AKS --updated --timeout 1200`으로 완료 대기 후 같은 명령 재실행 |
