...
| `order-service` | 주문 접수 API. 주문을 RabbitMQ로 발행(모듈 07 부하 대상) |
...
데이터 흐름: `virtual-customer → store-front/order-service → rabbitmq → makeline-service → mongodb`. virtual-customer 덕분에 별도 부하 도구 없이도 트래픽이 흐르며, 모듈 07에서 이 복제본 수를 늘려 부하를 키웁니다.
...
[03. 컨테이너 이미지 빌드](03-build-images.md)에서 ACR에 올린 이미지를 사용하도록, 매니페스트의 퍼블릭 이미지 경로(`ghcr.io/azure-samples/aks-store-demo/...`)를 우리 ACR 경로로 치환해 배포합니다. 모듈 03의 **기본 경로(라이트 4종 빌드)** 에 맞춰, **빌드한 4종만** ACR 경로로 치환하고 **Rust 3종(`product-service`·`virtual-customer`·`virtual-worker`)은 퍼블릭 이미지를 그대로** 사용합니다.
...
# 네임스페이스 생성(이미 있으면 그대로 사용 — 재실행해도 에러 없음)
kubectl create namespace pets --dry-run=client -o yaml | kubectl apply -f -

# 03에서 빌드한 '라이트 4종'만 ACR 경로로 치환(Rust 3종은 ghcr.io 퍼블릭 유지)
# 주의: BUILT 안의 '|'(OR)와 충돌하지 않도록 sed 구분자로 '#'을 사용합니다.
BUILT="order-service|makeline-service|store-front|store-admin"
sed -E "s#ghcr.io/azure-samples/aks-store-demo/(${BUILT}):[0-9.]+#${ACR_SERVER}/aks-store-demo/\1:${IMAGE_TAG}#g" \
  manifests/aks-store-all-in-one.yaml | kubectl apply -n pets -f -
```
- 치환은 **빌드한 4종**(`order-service`·`makeline-service`·`store-front`·`store-admin`)에만 적용되고, **Rust 3종**과 `mongodb`/`rabbitmq`/`busybox`는 퍼블릭 이미지를 그대로 씁니다.
...
> **전체 7종을 빌드했다면(대안):** 모든 앱 이미지를 ACR로 치환하려면 `BUILT` 없이 7종 전부를 치환합니다.
> ```bash
...
모듈 03의 ACR 빌드가 실패했거나(예: ACR Task 오류, 권한 문제) ACR에 이미지가 보이지 않으면, **치환 없이 퍼블릭 이미지(`ghcr.io`)를 그대로** 배포해 실습을 계속할 수 있습니다. 이 경우 ACR 대신 인터넷에서 이미지를 당겨오며, 기능/실습 결과는 동일합니다.
...
- `kubectl create namespace ... --dry-run=client -o yaml | kubectl apply -f -` 형태라 네임스페이스가 이미 있어도 `AlreadyExists` 에러 없이 진행됩니다.
...
```
virtual-customer가 자동 생성한 주문이 `store-front` → `order-service` → RabbitMQ 큐로 흐르는 과정을 로그로 확인합니다. order-service는 [Fastify](https://fastify.dev/) 기반 Node.js 앱이라 로그가 **JSON 한 줄(pino 포맷)** 로 찍힙니다.

📋 예상 출력:
```text
Defaulted container "order-service" out of: order-service, wait-for-rabbitmq (init)
{"level":30,"time":1782786115115,...,"reqId":"req-4w","req":{"method":"GET","url":"/health",...},"msg":"incoming request"}
{"level":30,"time":1782786115115,...,"reqId":"req-4w","res":{"statusCode":200},"responseTime":0.358...,"msg":"request completed"}
{"level":30,"time":1782786116746,...,"reqId":"req-4x","req":{"method":"POST","url":"/","host":"order-service:3000",...},"msg":"incoming request"}
sending message ${body} to ${process.env.ORDER_QUEUE_NAME} on ${process.env.ORDER_QUEUE_HOSTNAME} using local auth credentials
{"level":30,"time":1782786116747,...,"reqId":"req-4x","res":{"statusCode":201},"responseTime":1.682...,"msg":"request completed"}
{"level":30,"time":1782786118114,...,"reqId":"req-4y","req":{"method":"GET","url":"/health",...},"msg":"incoming request"}
{"level":30,"time":1782786118115,...,"reqId":"req-4y","res":{"statusCode":200},"responseTime":0.821...,"msg":"request completed"}
```

로그 읽는 법:
- **`Defaulted container "order-service" out of: order-service, wait-for-rabbitmq (init)`** — 이 Pod에는 컨테이너가 2개(앱 `order-service` + init `wait-for-rabbitmq`)라 `kubectl`이 앱 컨테이너를 자동 선택했다는 안내입니다(에러 아님). 특정 컨테이너를 보려면 `-c order-service`를 붙입니다.
- **`POST /` → `statusCode 201`** — virtual-customer가 보낸 **신규 주문 1건**이 정상 접수(201 Created)됐다는 핵심 신호입니다. 이 줄이 주기적으로 보이면 주문 흐름이 살아 있는 것입니다.
- **`sending message ... using local auth credentials`** — 접수한 주문을 RabbitMQ 큐로 발행하는 로그입니다. (앱이 템플릿 문자열을 그대로 출력해 `${body}`·`${process.env.ORDER_QUEUE_NAME}`가 치환되지 않은 채 보이지만, 동작에는 문제 없는 데모 앱의 로그 표현입니다.)
- **`GET /health` → `statusCode 200`** — 대부분을 차지하는 이 줄들은 kubelet의 **liveness/readiness probe** 트래픽입니다(주문과 무관, 정상 동작 증거).
- `level:30`은 pino의 `info` 레벨, `time`은 epoch 밀리초, `responseTime`은 처리 시간(ms)입니다.

> 정리: **`POST /` → `201` → `sending message ...(큐 발행)`** 패턴이 반복되면 주문 파이프라인이 정상입니다. 큐에 쌓인 주문은 makeline-service가 소비하며, 이 **큐 길이**를 모듈 07(KEDA)에서 오토스케일 트리거로 사용합니다.
...
- [ ] `pets` 네임스페이스가 생성됨
- [ ] **라이트 4종**(`order-service`·`makeline-service`·`store-front`·`store-admin`) Pod 이미지가 **ACR(`<acr>.azurecr.io/aks-store-demo/*:1.0.0`)** 에서 정상 pull됨. Rust 3종은 퍼블릭 `ghcr.io`에서 pull(전체 7종 빌드 시에는 7개 모두 ACR). (`kubectl get pods -n pets -o jsonpath` 또는 describe로 확인)
...
| `ImagePullBackOff`/`ErrImagePull` | ACR 이미지 미빌드/태그 불일치 또는 `AcrPull` 권한 | `kubectl describe pod <pod> -n pets`(Events의 이미지 경로 확인) | 모듈 03에서 해당 이미지(라이트 4종 또는 전체 7종)가 `1.0.0` 태그로 푸시됐는지(`az acr repository show-tags`), 배포 시 `ACR_SERVER`/`IMAGE_TAG` 치환이 맞는지 확인. 라이트 4종만 빌드했다면 Rust 3종은 ACR이 아닌 퍼블릭 `ghcr.io` 경로여야 정상. ACR `AcrPull` 역할(모듈 02) 확인 |
| Pod `Pending` 지속 | (정상) 첫 배포 시 NAP가 user 노드를 만드는 중, 또는 노드 자원 부족 | `kubectl describe pod <pod> -n pets` (Events의 `FailedScheduling`/`untolerated taint`), `kubectl get events -A --field-selector source=karpenter` | 첫 배포는 NAP 노드 생성으로 2~4분 `Pending`이 정상. 더 지속되면 NAP `mode=Auto`(모듈 02)·vCPU 쿼터 확인. 모듈 08에서 NAP 동작을 자세히 다룹니다 |
...
| `sed: ... unknown option to 's'` 후 `error: no objects passed to apply` | `sed`의 `s|...|` 구분자(`|`)가 `BUILT`의 OR(`|`)와 충돌 | 명령의 `sed -E "s| ... |"` 부분 확인 | 라이트 4종 치환은 **구분자를 `#`** 로 사용: `sed -E "s#ghcr.io/azure-samples/aks-store-demo/(${BUILT}):[0-9.]+#${ACR_SERVER}/aks-store-demo/\1:${IMAGE_TAG}#g"`. (위 2) 기본 경로 명령이 이미 `#` 사용) |
| `cd: ~/ms-aks-basic-workshop01: No such file or directory` | 클론 경로가 문서 예시(`~/ms-aks-basic-workshop01`)와 다름 | `pwd`로 현재 위치 확인 | 이미 저장소 루트에 있다면 `cd ~/ms-aks-basic-workshop01`은 생략하고 진행. 또는 실제 경로로 바꿔 실행 |
| `Error from server (AlreadyExists): namespaces "pets" already exists` | 네임스페이스를 이전에 이미 생성 | `kubectl get ns pets` | 무시 가능(정상). 본 문서는 `kubectl create namespace pets --dry-run=client -o yaml \| kubectl apply -f -` 형태라 재실행해도 에러가 나지 않습니다 |
