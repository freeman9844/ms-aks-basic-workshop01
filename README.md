...
- Managed Grafana/Prometheus와 Container Insights로 관측 — 또는 **(옵션)** OpenTelemetry 자동 계측으로 Application Insights 분산 추적([09.1](docs/09.1-autoinstrument-option.md))
...
| [03 컨테이너 이미지 빌드](docs/03-build-images.md) | 소스 → ACR (라이트 4종 `az acr build`) | **약 5–8분** | 기본=라이트 4종(약 2–3분)+검증. 전체 7종 빌드 시 병렬 8–10분(Rust `product-service`가 최장) |
...
| [06 Gateway API 트래픽 분할](docs/06-traffic-split-gateway.md) | HTTPRoute 가중치로 v1/v2 카나리 분할 | **8–12분** | 백엔드 v1/v2 기동 + 가중치 조정·관찰 |
| [07 오토스케일링 (1) KEDA](docs/07-autoscaling-keda.md) | RabbitMQ 큐 트리거로 Pod 0↔N | **10–15분** | 큐 적재→확장, scale-to-zero 쿨다운 대기 |
| [08 오토스케일링 (2) NAP](docs/08-autoscaling-nap.md) | NAP로 노드 자동 프로비저닝 | **12–18분** | 신규 노드 프로비저닝 1–3분 + scale-in 대기 |
| [09 모니터링](docs/09-monitoring.md) | Grafana/Prometheus + Container Insights/KQL | **12–18분** | 대시보드·Explore·KQL, 로그 수집 지연 1–5분 |
| ┗ [09.1 **(옵션)** 자동 계측](docs/09.1-autoinstrument-option.md) | OpenTelemetry 자동 계측 + Application Insights 트레이스 | (09 추가) | App Insights 생성 + 애드온 활성화 3–7분(시간 여유 시) |
| [10 정리](docs/10-cleanup.md) | terraform destroy | **8–12분** | AKS·Grafana 삭제에 수 분 소요 |
...
## 예상 비용
Korea Central, 약 2~2.5시간 1회 기준 소액(수천 원 수준 — AKS 노드 + Managed Grafana + Standard LB + 로그/메트릭 수집). **실습 후 반드시 [10. 정리](docs/10-cleanup.md)의 `terraform destroy`를 실행하세요.**
...
| Gateway 트래픽 분할(가중치·라우팅 미반영) | [06. 트래픽 분할](docs/06-traffic-split-gateway.md#트러블슈팅) |
| 오토스케일링 KEDA(ScaledObject·큐 트리거) | [07. KEDA](docs/07-autoscaling-keda.md#트러블슈팅) |
| 오토스케일링 NAP(Pending·노드 생성·scale-in) | [08. NAP](docs/08-autoscaling-nap.md#트러블슈팅) |
| 모니터링(Grafana·Prometheus·ContainerLogV2) | [09. 모니터링](docs/09-monitoring.md#트러블슈팅) · [09.1 자동 계측 **(옵션)**](docs/09.1-autoinstrument-option.md#트러블슈팅) |
| 정리/삭제(destroy 실패·잔여 RG) | [10. 정리](docs/10-cleanup.md#트러블슈팅) |
